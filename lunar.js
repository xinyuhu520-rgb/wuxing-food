...
function solar2lunar(y,m,d) {
  ...
  var gan = Gan[(year-4)%10];
  var zhi = Zhi[(year-4)%12];
  ...
  return {
    ...
    animal: animal,
    yearCn: gan + zhi + '年',
    monthCn: (isLeap?'闰':'') + getMonthCn(month),
    dayCn: getDayCn(day)
  };
}

// 农历月中文
function getMonthCn(m) {
  var monthCn = ['正','二','三','四','五','六','七','八','九','十','冬','腊'];
  return monthCn[m-1] + '月';
}

// 农历日中文
function getDayCn(d) {
  var dayCn = ['初一','初二','初三','初四','初五','初六','初七','初八','初九','初十',
